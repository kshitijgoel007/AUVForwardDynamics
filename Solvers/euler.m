  ...
  function Y=euler(caseNo,timespan,X)
...
% SAVING Y VECTOR
  fname1=['generated\excel data\case_',caseNo,'.csv'] ;
  fid=fopen(fname1,'w') ;
  dlmwrite(fname1,Y);
  fclose(fid);
end
...
% Function implementing turn in horizontal plane with constant rudder deflection 
  for i=2:length(timespan)
      % SOLVING USING EULER METHOD
        DX=forwarddynamics2(timespan(i),X,ord_defl,caseNo);
        temp=X+DX*dt;
        X=temp;
        Y(i,1)=timespan(i);
        for j=1:length(X)
...
%*************************************************************************
 ...
 for i=2:length(timespan)
      ...
      % EULER METHOD IMPLEMENTED
        ...
        if (X(12)>abs(ord_defl(1))) 
            ord_defl(1)=abs(ord_defl(1));
        else if (X(12)<-abs(ord_defl(1)))
            ord_defl(1)=-abs(ord_defl(1));    
            end
        end
...
%*******************************************************************
 ...
 for i=2:length(timespan)
      ...
      % EULER METHOD IMPLEMENTED
        ...
        if (X(11)>abs(ord_defl(2))) 
            ord_defl(2)=abs(ord_defl(2));
        else if (X(11)<-abs(ord_defl(2)))
            ord_defl(2)=-abs(ord_defl(2));    
            end
        end
      % NOTE : ruddder deflection to starboard side is -ve
