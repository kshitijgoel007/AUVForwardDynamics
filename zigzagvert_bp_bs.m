...
%       % del_s=-10*pi/180.0;
  ...
  saveas(figure(1),'E:\AUV\AUVForwardDynamics\Results\vert_zigzag_bp_bs\const_zero\euler.fig');
   ...
   figure(3);
   subplot(2,1,1);plot(t, Y(:,14));xlabel('time');ylabel('\delta_{b_{s,actual}}');
   subplot(2,1,2);plot(t, Y(:,14));xlabel('time');ylabel('\delta_{bp_{p,actual}}');
